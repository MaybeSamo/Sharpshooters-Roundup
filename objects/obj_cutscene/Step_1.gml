...
if (wait_if) {
    if (scr_compare(value_to_comp, comp, value_to_comp2)) {
        event_index += 1;
        wait_if = false;
    }
}
...
switch(cmd) {
    ...
    case "set_siner_values":
        ...
        break;
    case "c_wait_if":
        value_to_comp = command[1];
        comp = command[2];
        value_to_comp2 = command[3];
        break;
    case "set_speed":
        active_actor.hspeed = command[1];
        active_actor.vspeed = command[2];
        break;
    case "move_towards_point":
        scr_move_towards_point(active_actor, command[1], command[2], command[3]);
        break;
}
if (!waiting and !text_waiting and !wait_if) {
    event_index++;
}